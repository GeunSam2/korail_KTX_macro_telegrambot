...
from gui.worker import ReservationRequest, ReservationWorker
...
class MainWindow(QMainWindow):
    ...
    def _notification_tab(self) -> QWidget:
        ...
        group = QGroupBox("이메일 알림 (준비 중)")
        group_layout = QVBoxLayout(group)
        info = QLabel("이메일 알림은 현재 비활성화되어 있습니다.\n예약 성공·실패는 기본으로 Windows 알림 센터와 앱 로그에 표시됩니다.")
    ...
    def _load_settings(self) -> None:
        ...
        try:
            ...
            self.password.setText(self.credentials.get("korail_password"))
    ...
    def closeEvent(self, event: QCloseEvent) -> None:
        if self.thread and self.thread.isRunning():
            QMessageBox.warning(self, "예약 실행 중", "먼저 예약 검색을 중지한 뒤 종료하세요."); event.ignore(); return
